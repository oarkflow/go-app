...
// StartAsConsumer makes a node act as a consumer and connect to a Task Manager.
func (n *Node) StartAsConsumer(taskManagerAddr string) error {
	conn, err := net.Dial("tcp", taskManagerAddr)
	if err != nil {
		return fmt.Errorf("failed to connect to task manager: %v", err)
	}
	n.conn = conn
	fmt.Println("Connected to Task Manager at", taskManagerAddr)
...
// listenForTasks listens for incoming tasks from the Task Manager.
func (n *Node) listenForTasks() {
	...
	for {
		...
		// Process the task using the node's handler
		ctx := context.Background()
		result := n.handler(ctx, &task)

		// Send the result back to Task Manager
		n.sendResult(result)
	}
}

// sendResult sends the task result back to the Task Manager.
func (n *Node) sendResult(result Result) {
...
func handler3(ctx context.Context, task *Task) Result {
	var user map[string]any
	json.Unmarshal(task.Payload, &user)
...
func main() {
	// Distributed mode Task Manager
	tm := NewTaskManager(Distributed)
	tm.AddNode("A", handler1)
	tm.AddNode("B", handler2)
	tm.AddNode("C", handler3)
	tm.AddEdge("A", "B", LoopEdge)
	tm.AddEdge("B", "C", SimpleEdge)
	go tm.StartServer(":9000") // Start Task Manager on port 9000
	...
	http.HandleFunc("/publish", func(w http.ResponseWriter, r *http.Request) {
		...
		// Process the task (send to consumer)
